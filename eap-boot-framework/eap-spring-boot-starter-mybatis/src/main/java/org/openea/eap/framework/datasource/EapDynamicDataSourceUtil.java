...
public class EapDynamicDataSourceUtil {
    ...
    public static DataSource getDataSource(String ds){
        ...
        if (dynamicRoutingDataSource == null) {
            dynamicRoutingDataSource = SpringUtils.getBean(DynamicRoutingDataSource.class);
            defaultDataSourceCreator = SpringUtils.getBean(DefaultDataSourceCreator.class);
        }
        if(dynamicRoutingDataSource==null){
           // throw exception
        }
