...
@Service
@ConditionalOnMissingBean(DictDataApi.class)
@FeignClient(name = ApiConstants.NAME) // TODO 芋艿：fallbackFactory =
@Tag(name = "RPC 服务 - 字典数据")
public interface DictDataApiClient extends DictDataApi {
    ...
    Boolean validateDictDataList(@RequestParam("dictType") String dictType,
    ...
    DictDataRespDTO getDictData(@RequestParam("dictType") String dictType,
    ...
    DictDataRespDTO parseDictData(@RequestParam("dictType") String dictType,
                                                @RequestParam("label") String label);

    @GetMapping(PREFIX + "/list")
    @Operation(summary = "获得指定字典类型的字典数据列表")
    @Parameter(name = "dictType", description = "字典类型", example = "SEX", required = true)
    List<DictDataRespDTO> getDictDataList(@RequestParam("dictType") String dictType);
